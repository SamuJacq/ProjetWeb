...
if(isset($_POST['send'])){
    ...
    if(!$nameUser){
        ...
        if($nameOk){
            ...
            if($depense->UpdateDepense($insert, $m)){
...
<main class="formulaireCentre">
    <form action="depenseUpdate.php<?php echo"?id=$id"?>" method="POST" enctype ="application/x-www-form-urlencoded">
        <?php
            if(isset($_POST['update'])){
                echo " 
                        ...
                        <label for=\"date\"> date* </label><input id=\"libelle\" type=\"date\" name=\"libelle\" required value=\"date('dd/mm/yyyy', time())\">
                ...
                foreach($allDepense as $infoDepense){
                    echo "
                       <p>persone à dépenser $infoDepense->montant € le ". date('Y-m-d', $infoDepense->dateheure)."</p>
